...
# ---------- CF Turnstile 等待 ----------
def wait_cf_turnstile(page, timeout=60, check_renew_modal=True) -> bool:
    log.info("检查 Cloudflare Turnstile 验证...")

    if check_renew_modal:
    ...
    while time.time() < deadline:
        ...
        if not still_verifying:
            log.info("✅ CF Turnstile 验证未发现或已完成")
            return True
...
def login(page, max_retries=3) -> bool:
    ...
    user_selector = 'input[name="identifier"], input[name="username"], input[name="email"], input[type="email"], input[autocomplete="username email"]'
    pass_selector = 'input[name="password"], input[type="password"], input[autocomplete="current-password"]'

    for attempt in range(1, max_retries + 1):
        log.info(f"登录 {attempt}/{max_retries} - 正在导航至 {login_url}")
        try:
        ...
        try:
            if page.locator(pass_selector).first.is_visible():
                log.info("检测到密码框，填写密码（单页模式）")
                ...
                log.info("未检测到密码框，点击下一步（分步模式）")
                page.locator('button[type="submit"], button:has-text("Continue")').first.click()
                
                page.wait_for_selector(pass_selector, timeout=15000)
                log.info("已进入密码输入页")
            ...
            log.warning(f"填写密码或点击提交时失败: {e}")
            take_screenshot(page, f"login_password_step_error_{attempt}")
            continue

        # --- 修复后的跳转等待逻辑 ---
        log.info("⏳ 等待页面跳转（最多 25 秒）...")
        redirected = False
        for _ in range(25):
            time.sleep(1)
            # 只要 URL 发生了改变，不再是登录页，且包含了 zampto，即视为跳转成功
        ...
        if redirected:
            ...
            log.warning(f"⚠️ 登录后未正确跳转，当前 URL: {page.url}")
            take_screenshot(page, f"login_fail_{attempt}")
            
            # 有时点击登录后会弹出 Cloudflare 验证，检查并等待
            log.info("检查是否被 Cloudflare 拦截...")
            wait_cf_turnstile(page, timeout=15, check_renew_modal=False)
...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    ...
    if not wait_cf_turnstile(page, timeout=60, check_renew_modal=True):
