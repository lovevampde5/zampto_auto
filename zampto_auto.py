...
def parse_expiry_minutes(expiry_str: str) -> int:
    """解析到期时间字符串为分钟数，支持多种格式"""
    if not expiry_str:
        return -1
    # 尝试匹配绝对时间（如 2025-01-01 00:00:00）
    try:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            page.wait_for_selector(
                'input[name="identifier"], input[autocomplete="username email"], input[type="email"], input[type="text"]',
                timeout=15000
            )
        except:
...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    ...
    def log_req(request):
        ...
        if any(kw in url.lower() for kw in ["renew", "extend", "refresh"]) and method in ("GET", "POST", "PUT"):
            post_data = None
            req_headers = None
            try:
            ...
            captured_renew_req["method"] = method
            captured_renew_req["url"] = url
            captured_renew_req["post_data"] = post_data
            captured_renew_req["headers"] = req_headers
    ...
    def click_modal_confirm(page) -> bool:
        result = page.evaluate("""() => {
            ...
            var confirmTexts = ['Confirm', 'Yes', 'OK', 'Ok', 'Submit', 'Renew', 'Renew Server', 'Extend', 'Continue'];
            ...
            for (var b of btns) {
                ...
                if ((txt.includes('confirm') || txt.includes('renew') || txt.includes('extend')) && b.offsetParent !== null) {
                    b.scrollIntoView({block: 'center'});
                    b.click();
                    return 'partial:' + txt;
                }
    ...
    for attempt in range(1, 4):
        log.info(f"🔄 [方法1] 尝试 {attempt}/3 - 点击 Renew Server 按钮")
        try:
            page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        except:
            continue
        ...
        try:
            btn_clicked = page.evaluate("""() => {
                ...
                for (var el of els) {
                    ...
                    if (txt === 'Renew Server' || txt.includes('Renew Server') || txt === 'Renew' || txt === 'Extend') {
            ...
            if btn_clicked:
                log.info("✅ 已通过 JS 点击 Renew Server")
            else:
                ...
                if renew_btn.count():
                    ...
                    log.info("✅ 已通过 locator 点击续期按钮")
                    btn_clicked = True
                else:
                    log.warning(f"第 {attempt} 次：未找到任何续期按钮，截图后继续")
                    take_screenshot(page, f"renew_no_btn_attempt{attempt}")
                    continue
        except Exception as e:
        ...
        if modal_visible:
            ...
            if not confirmed:
                try:
                    ...
                    log.info("  回退：按 Enter 键")
                    time.sleep(2)
                except:
                    pass
        else:
            log.info("无模态框，可能已直接触发或触发失败")

        log.info("等待 20 秒监听网络请求...")
        time.sleep(20)

        if captured_renew_req["url"]:
            log.info(f"🎯 检测到捕获的续期 API，尝试重放: {captured_renew_req['url']}")
            try:
                ...
                if captured_renew_req["headers"]:
                    for k, v in captured_renew_req["headers"].items():
                        if k.lower() not in ("host", "content-length"):
                            headers[k] = v
                method = captured_renew_req["method"]
                url = captured_renew_req["url"]
                post_data = captured_renew_req["post_data"]
                if method == "GET":
    ...
    try:
        ...
        result = page.evaluate("""() => {
            ...
            var app = document.querySelector('#app') || document.querySelector('[data-server-id]');
            if (app && app.__vue__) {
                var vm = app.__vue__;
        ...
        if result:
            log.info(f"  调用了 {result}，等待 15 秒检查 expiry...")
            time.sleep(15)
    ...
    log.info("=====================================")
    if captured_renew_req["url"]:
        log.info(f"🎯 已识别的疑似续期 API（但重放未成功）: {captured_renew_req['method']} {captured_renew_req['url']}")
    else:
