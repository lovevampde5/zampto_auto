...
def redact_sensitive_info(page):
    ...
    try:
        page.evaluate(r"""() => {
            document.querySelectorAll('.user-info-grid .info-card .info-content p').forEach(p => p.textContent = '***');
            var addrEl = document.getElementById('addressValue');
            if (addrEl) addrEl.textContent = '***';
            document.querySelectorAll('.info-card-value').forEach(el => {
...
# === 核心修复点：重构过后的超精简防广告弹窗点击逻辑（避免手机浏览器篡改和截断） ===
def dismiss_all_popups(page):
    for round_idx in range(4):
        try:
            # 1. 隐藏广告框架和浮动元素
            page.evaluate(r"""() => {
                document.querySelectorAll('iframe[id*="google_vignette"], iframe[id*="aswift"], ins.adsbygoogle').forEach(el => el.style.setProperty('display', 'none', 'important'));
                document.querySelectorAll('div[style*="position: fixed"], div[style*="position:fixed"]').forEach(el => {
                    var z = parseInt(window.getComputedStyle(el).zIndex) || 0;
                    if (z >= 9000 && !el.id.includes('renew') && !el.id.includes('modal')) el.style.setProperty('display', 'none', 'important');
                });
            }""")
            ...
            closed = page.evaluate(r"""() => {
                var count = 0;
                var selectors = [
                    'button[aria-label="Close"]', 'button[aria-label="close"]', '.close-button-protector', '.dismiss-button',
                    'button:has-text("Close")', 'button:has-text("close")', 'a:has-text("×")', 'button:has-text("×")'
                ];
                selectors.forEach(s => {
                    try {
            ...
            if closed and closed > 0:
                log.info(f"  [轮{round_idx+1}] 自动关闭了 {closed} 个干扰弹窗")
                time.sleep(1)
            else:
                break
        except Exception as e:
...
def get_server_info(page, server_id: str) -> dict:
    ...
    try:
        ...
        status_text = page.evaluate(r"""() => {
            ...
            return runEl ? runEl.innerText.trim() : 'Unknown';
        }""")
...
def main():
    ...
    try:
        ...
        msg = f"🖥️ **Zampto 服务器监控**\n状态: {status}\n到期时间: `{new_expiry}`\n自动续期: {'成功 ✅' if renewed else '跳过或未触发 ⏳'}"
        wxpush(msg)
        tgpush(msg)
