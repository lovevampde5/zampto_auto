...
# ---------- 续期（核心增强版） ----------
def renew_server(page, server_id: str, expiry_before: str, retry=True) -> bool:
    """
    执行续期流程，包含模态框确认按钮智能定位和 CF 处理。
    retry: 若第一次失败，是否自动重试一次。
    """
    server_url = f"{BASE_URL}/server?id={server_id}"

    def _do_renew():
        log.info("准备续期，访问服务器详情页")
        try:
            page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        except Exception as e:
        ...
        if not modal_visible:
            log.warning("⚠️ 未检测到续期模态框，可能续期已直接触发（无确认步骤）")
            # 仍尝试处理可能出现的 CF
            if handle_cf_turnstile(page, timeout=30):
                log.info("✅ CF 处理完成（或无验证），等待续期结果...")
                time.sleep(8)
                return True  # 假设成功，由外层判断 expiry
            else:
                return False

        log.info("✅ 续期模态框已出现，开始查找确认按钮...")
        ...
        log.info(f"模态框内按钮文本: {btn_info}")

        # 定义关键词列表（优先顺序）
        keywords = ["Renew", "Confirm", "Submit", "Extend", "Add", "续期", "确认", "OK", "Yes", "继续"]
        target_btn = None
        for kw in keywords:
            try:
                # 尝试多种选择器
                selector = f'#renewModal button:has-text("{kw}"), #renewModal a:has-text("{kw}"), #renewModal input[value*="{kw}"]'
                btn = page.locator(selector).first
                if btn.count() and btn.is_visible():
                    target_btn = btn
                    log.info(f"  找到匹配 '{kw}' 的按钮")
                    break
            except:
                pass

        # 如果仍未找到，尝试点击模态框内第一个可见的按钮（排除关闭按钮）
        if not target_btn:
            log.info("  未找到关键词匹配，尝试点击模态框内第一个非关闭按钮...")
            all_btns = page.locator('#renewModal button, #renewModal a, #renewModal input[type="submit"]').all()
            for b in all_btns:
                if b.is_visible():
                    txt = (b.inner_text() or "").strip()
                    if txt.lower() not in ["×", "close", "x", "关闭"]:
                        target_btn = b
                        log.info(f"  选择按钮文本: '{txt}'")
                        break
        ...
        if target_btn:
            try:
                ...
                log.warning(f"点击确认按钮失败: {e}")
                # 尝试 JS 点击
                try:
                    page.evaluate("""(btn) => btn.click()""", target_btn)
                    log.info("  使用 JS 点击成功")
                except:
                    log.warning("  点击确认按钮最终失败")
                    return False
        else:
            log.warning("⚠️ 未找到任何可点击的确认按钮，尝试直接处理 CF（可能续期已自动触发）")
            # 可能无需确认，直接进入验证

        # ---------- 步骤3：处理 CF Turnstile ----------
        if not handle_cf_turnstile(page, timeout=60):
            log.warning("❌ CF 验证处理失败，续期可能未完成")
            take_screenshot(page, "08_cf_fail")
            return False

        log.info("⏳ 等待续期请求完成...")
        time.sleep(8)
        take_screenshot(page, "09_after_renew")
        return True
    ...
    for attempt in range(1, 3 if retry else 2):
        ...
        if success:
            ...
            if minutes_after > minutes_before:
                ...
                log.warning(f"⚠️ 第 {attempt} 次续期后 expiry 未增加，可能失败")
            ...
            log.warning(f"❌ 第 {attempt} 次续期流程执行失败")
            if attempt == 1 and retry:
                log.info("将进行第二次重试...")
                time.sleep(5)
                continue
...
def main():
    ...
    try:
        ...
        if SKIP_RENEW:
            ...
            renewed = renew_server(page, SERVER_ID, expiry_before=expiry, retry=True)
