...
# ---------- ★★★ 核心修改：续期函数（基于网络监听，不点击确认按钮） ----------
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    """
    使用网络请求监听来判断续期是否成功。
    不再点击模态框内的任何按钮（包括 Cancel），避免中断请求。
    """
    server_url = f"{BASE_URL}/server?id={server_id}"

    # 用于记录续期请求是否发出
    renew_request_found = False
    renew_response_ok = False

    def handle_request(request):
        nonlocal renew_request_found
        url = request.url.lower()
        # 捕获可能的续期 API，例如包含 renew, extend, server 等
        if "renew" in url or "extend" in url or ("server" in url and "update" in url):
            log.info(f"📡 检测到可能的续期请求: {request.method} {request.url}")
            renew_request_found = True

    def handle_response(response):
        nonlocal renew_response_ok
        if "renew" in response.url.lower() or "extend" in response.url.lower():
            if response.ok:
                log.info(f"✅ 续期响应成功: {response.status} {response.url}")
                renew_response_ok = True
    ...
    for attempt in range(1, 3):  # 最多重试 2 次
        ...
        # 等待模态框出现（最多 10 秒），但 **不点击任何按钮**
        modal_seen = False
        for _ in range(10):
            modal_visible = page.evaluate("""() => {
                var m = document.getElementById('renewModal');
                if (m) return m.offsetParent !== null || m.style.display !== 'none';
            ...
            if modal_visible:
                ...
                btn_texts = page.evaluate("""() => {
                    ...
                    if (!modal) return [];
                    var items = modal.querySelectorAll('button, a, input[type="submit"], [role="button"]');
                    var texts = [];
                    for (var el of items) {
                ...
                log.info(f"模态框内按钮文本: {btn_texts}")
                break
            time.sleep(1)
        if not modal_seen:
            log.warning("⚠️ 未检测到续期模态框，可能续期已直接触发")

        # 处理可能出现的 CF Turnstile（如果有）
        cf_ok = handle_cf_turnstile(page, timeout=40)
        if not cf_ok:
            log.warning("❌ CF 验证处理失败，但可能不影响续期，继续...")

        # 等待一段时间，让请求完成
        log.info("⏳ 等待续期请求完成（10 秒）...")
        time.sleep(10)
        take_screenshot(page, f"after_renew_{attempt}")
        ...
        if minutes_after > minutes_before:
            ...
            return True
        elif renew_request_found or renew_response_ok:
            log.info(f"✅ 检测到续期网络请求，但 expiry 未刷新，可能需等待更长时间，视为成功")
            # 再等待一段时间再检查一次
            time.sleep(5)
            page.reload()
            ...
            if parse_expiry_minutes(info_after2) > minutes_before:
                ...
                log.warning("有续期请求但 expiry 最终未增加，可能请求失败")
        else:
            log.warning(f"⚠️ 第 {attempt} 次续期未成功（无网络请求且 expiry 未增加）")

        # 如果还有重试机会，关闭模态框（点击 Cancel）并重新开始
        if attempt < 2:
            log.info("尝试关闭模态框（点击 Cancel）并重试...")
            try:
                cancel_btn = page.locator('#renewModal button:has-text("Cancel"), #renewModal button:has-text("cancel")').first
                if cancel_btn.count() and cancel_btn.is_visible():
                    cancel_btn.click()
                    log.info("已点击 Cancel 关闭模态框")
                    time.sleep(2)
                else:
                    # 尝试点击模态框外区域或按 ESC
                    page.keyboard.press("Escape")
                    log.info("按 ESC 键尝试关闭模态框")
                    time.sleep(2)
            except Exception as e:
                log.warning(f"关闭模态框失败: {e}")
            # 继续循环
