...
def tg_push(content: str):
    ...
    payload = json.dumps({
        "chat_id": TG_CHAT_ID,
        "text": content,
        "parse_mode": "HTML" if "</b>" in content or "🟢" in content else ""
    }).encode('utf-8')
...
def broadcast_notification(content: str):
    """统一通知分发"""
    wxpush(content)
    tg_push(content)
...
def parse_expiry_minutes(expiry_str: str) -> int:
    if not expiry_str or "未知" in expiry_str:
        return -1
    total = 0
...
def main():
    ...
    if not SERVER_ID:
        ...
        broadcast_notification("❌ 未配置 ZAMPTO_SERVER_ID，任务中止")
        return
    ...
    try:
        if not login(page):
            broadcast_notification("❌ Zampto 登录失败，请检查账号密码")
            return
        ...
        broadcast_notification(f"❌ Zampto 任务异常: {e}")
    finally:
        time.sleep(3)
        browser.close()
        log.info("任务结束")
