...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    ...
    采用多维度尝试触发续期，包括：
    - 点击 Renew Server 按钮
    - 在模态框中查找所有可点击元素（忽略 Cancel），点击匹配关键词或第一个
    - 模拟 Enter 键
    - 点击模态框空白区域
    - 同时监听网络请求
    - 处理 reCAPTCHA
    ...
    def log_request(request):
        url = request.url
        if any(k in url.lower() for k in ['renew', 'extend', 'server/update', '/api/', 'server?id']):
            log.info(f"📡 请求: {request.method} {url}")
            captured_requests.append(url)

    page.on("request", log_request)

    for attempt in range(1, 3):  # 最多尝试 2 次
        log.info(f"🔄 续期尝试 {attempt}/2")
        # 访问详情页
        try:
            page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        except Exception as e:
        ...
        try:
            ...
            if not btn_clicked:
                log.warning("未找到 Renew Server 按钮，尝试 Playwright locator...")
                ...
                if renew_btn.count():
                    ...
                    log.error("无法定位 Renew Server 按钮")
                    continue
            else:
                log.info("已点击 Renew Server 按钮（原生 JS）")
            take_screenshot(page, f"renew_clicked_{attempt}")
        except Exception as e:
        ...
        for _ in range(10):
            modal_visible = page.evaluate("""() => {
                ...
                if (m && (m.offsetParent !== null || m.style.display !== 'none')) return true;
                var modals = document.querySelectorAll('[class*="modal"][id*="renew"], [class*="modal"][class*="renew"]');
                for (var md of modals) {
                    if (md.offsetParent !== null) return true;
                }
                return false;
            }""")
            if modal_visible:
            ...
            time.sleep(1)
        if not modal_seen:
            log.warning("⚠️ 未检测到续期模态框，可能续期已直接触发")
            # 仍继续尝试检查 expiry

        # 处理可能出现的验证码
        handle_google_recaptcha(page)  # 尝试点击 reCAPTCHA
        handle_cf_turnstile(page, timeout=30)

        # ★★★ 关键：在模态框内寻找确认按钮（排除 Cancel） ★★★
        if modal_seen:
            log.info("🔍 在模态框中寻找确认按钮...")
            # 使用 JS 获取模态框内所有可点击元素的信息
            elements_info = page.evaluate("""() => {
                var modal = document.getElementById('renewModal') || document.querySelector('[class*="modal"][id*="renew"], [class*="modal"][class*="renew"]');
                if (!modal) return [];
                var items = modal.querySelectorAll('button, a, input[type="submit"], [role="button"], [role="link"], .btn, [class*="btn"]');
                var result = [];
                for (var el of items) {
                    var txt = (el.innerText || el.textContent || el.value || '').trim();
                    var aria = el.getAttribute('aria-label') || '';
                    var title = el.getAttribute('title') || '';
                    var data = el.getAttribute('data-action') || '';
                    var id = el.id || '';
            ...
            log.info(f"模态框内元素: {elements_info}")

            # 定义关键词（不区分大小写）
            keywords = ['renew', 'extend', 'confirm', 'ok', 'yes', 'proceed', 'continue', 'add', 'submit', '确认', '续期', '延长']
            target_el = None
            for info in elements_info:
                if not info['visible']:
                    continue
                combined = (info['text'] + ' ' + info['aria'] + ' ' + info['title'] + ' ' + info['dataAction']).lower()
                if any(kw in combined for kw in keywords):
                    target_el = info
                    log.info(f"✅ 找到匹配关键词的元素: {info}")
                    break
            # 如果没找到，选择第一个可见且文本不为 'Cancel' 的元素
            if not target_el:
                for info in elements_info:
                    if info['visible'] and info['text'].lower() != 'cancel':
                        target_el = info
                        log.info(f"选择第一个非 Cancel 元素: {info}")
                        break

            if target_el:
                # 尝试点击该元素（使用 locator 定位）
                try:
                    # 通过文本和属性组合定位
                    selector_parts = []
                    if target_el['id']:
                        selector_parts.append(f"#{target_el['id']}")
                    if target_el['text']:
                        # 使用 XPath 包含文本
                        xpath = f"//*[text()='{target_el['text']}']"
                        try:
                            page.locator(f"xpath={xpath}").click(timeout=5000)
                            log.info(f"✅ 点击了元素（XPath）: {target_el['text']}")
                            time.sleep(3)
                            break  # 点击后跳出
                        except:
                            pass
                    ...
                    if selector_parts:
                        try:
                            page.locator(selector_parts[0]).click(force=True, timeout=5000)
                            log.info(f"✅ 点击了元素（ID/Class）: {selector_parts[0]}")
                            time.sleep(3)
                            break
                        except:
                            pass
                    # 兜底：尝试点击模态框本身
                    log.info("尝试点击模态框内容区域...")
                    page.click('#renewModal', force=True)
                    time.sleep(3)
                except Exception as e:
                    log.warning(f"点击目标元素失败: {e}")
            else:
                log.warning("未找到任何可点击的确认元素，尝试按 Enter 键...")
                page.keyboard.press("Enter")
                time.sleep(2)

        # 等待网络请求完成
        log.info("⏳ 等待续期请求（最多 30 秒）...")
        time.sleep(5)
        ...
        log.info(f"续期后 expiry: {info_after} (分钟: {minutes_after})")

        # 判断成功
        if minutes_after > minutes_before:
            log.info(f"✅ 续期成功！expiry 从 {expiry_before} 增加到 {info_after}")
            page.remove_listener("request", log_request)
            return True

        # 如果捕获到任何包含 'renew' 的请求且未成功，可能请求已发出但未刷新，再等待一轮
        if captured_requests and any('renew' in req for req in captured_requests):
            log.info("检测到续期请求，但 expiry 未刷新，等待 10 秒后再检查...")
            time.sleep(10)
            page.reload()
            info_after2 = page.evaluate("""() => {
                var body = document.body.innerText || '';
                var m = body.match(/Expiry[^:]*:\\s*([^\\n]+)/i);
                return m ? m[1].trim() : null;
            }""")
            if parse_expiry_minutes(info_after2) > minutes_before:
                log.info("✅ 刷新后 expiry 增加，确认成功")
                page.remove_listener("request", log_request)
                return True

        log.warning(f"⚠️ 第 {attempt} 次续期未成功")
        # 尝试关闭模态框（按 ESC）
        try:
