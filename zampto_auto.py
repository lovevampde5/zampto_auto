...
def handle_cf_turnstile(page, timeout=90) -> bool:
    ...
    for _ in range(10):
        ...
        time.sleep(0.5)
    if not appeared:
        log.info("✅ 5s 内未出现 Cloudflare iframe，视为无需验证或已自动通过")
        return True
    log.info("🔄 检测到 Cloudflare Turnstile iframe，开始多策略处理...")
    try:
        log.info("  [策略A] frame_locator 点击复选框...")
        ...
        log.info(f"  策略A 异常: {e}")
    try:
        log.info("  [策略B] 点击 iframe 左侧复选框位置...")
        ...
        if iframe_el.count() > 0:
            ...
            if box:
                ...
                time.sleep(0.3)
                page.mouse.click(box['x'] + box['width'] * 0.15, box['y'] + box['height'] / 2)
                for _ in range(20):
                    time.sleep(1)
                    if page.locator(cf_iframe_selector).count() == 0:
                        log.info("✅ [策略B] 验证通过")
        ...
        log.info(f"  策略B 异常: {e}")
    try:
        log.info("  [策略C] JS 注入点击...")
        page.evaluate(f"""() => {{
            var iframe = document.querySelector('{cf_iframe_selector}');
            if (iframe) {{
                try {{
                    var doc = iframe.contentDocument || iframe.contentWindow.document;
                    var cb = doc.querySelector('[role="checkbox"], .challenge, .checkbox, input[type="checkbox"]');
                    if (cb) {{ cb.click(); return true; }}
                }} catch(e) {{ iframe.click(); }}
            }}
        }}""")
        for _ in range(10):
            time.sleep(1)
        ...
        log.info(f"  策略C 异常: {e}")
    log.info(f"  [策略D] 等待自动验证完成（最长 {timeout}s）...")
    deadline = time.time() + timeout
    while time.time() < deadline:
        if page.locator(cf_iframe_selector).count() == 0:
            log.info("✅ [策略D] CF iframe 已消失，验证通过")
            return True
        time.sleep(2)
    log.warning("⚠️ CF Turnstile 所有策略均未能确认通过，继续执行")
    take_screenshot(page, "cf_turnstile_uncertain")
    return True
        ...
        try:
            page.evaluate("""() => {
                ...
                if (iframe) {
                    try {
                        var doc = iframe.contentDocument || iframe.contentWindow.document;
                        var cb = doc.querySelector('.recaptcha-checkbox-border');
                        if (cb) cb.click();
                    } catch(e) { iframe.click(); }
                }
            }""")
...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    ...
    all_responses = []
    captured_renew_reqs = []
    captured_renew_req = {"method": None, "url": None, "post_data": None, "headers": None}
    ...
    def log_req(request):
        ...
        if any(kw in url.lower() for kw in ["renew", "extend", "refresh", "subscription"]) and method in ("GET", "POST", "PUT", "PATCH"):
            post_data = None; req_headers = None
            try: post_data = request.post_data; req_headers = dict(request.headers)
            except: pass
            if post_data: log.info(f"    POST数据: {post_data[:300]}")
            entry = {"method": method, "url": url, "post_data": post_data, "headers": req_headers}
            captured_renew_reqs.append(entry)
            captured_renew_req.update(entry)
    ...
    def check_expiry_increased() -> bool:
        try:
            page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
            time.sleep(4); dismiss_all_popups(page); time.sleep(1)
        except: pass
        ...
        log.info(f"  当前 expiry: {info_after}（续期前: {expiry_before}）")
        if info_after is None: return False
        if not expiry_before or expiry_before in ("未知", "Unknown", ""):
            log.info("  续期前 expiry 未知，以有值视为成功"); return True
        return parse_expiry_minutes(info_after) > parse_expiry_minutes(expiry_before)

    def sniff_renew_api(page) -> list:
        found = []
        try:
            html = page.content()
            patterns = [
                ...
                for m in re.finditer(pat, html, re.I):
                    ...
                    if any(kw in path.lower() for kw in ["renew", "extend", "refresh"]):
                        ...
                        if full not in found:
                            found.append(full)
                            log.info(f"  🔍 嗅探到候选 API: {full}")
        except Exception as e:
            log.warning(f"嗅探 API 失败: {e}")
        return found

    def build_api_session(page):
        ...
        headers = {
            ...
            "Referer": f"{BASE_URL}/server?id={server_id}", "Origin": BASE_URL,
        }
        if csrf_token: headers["X-CSRF-Token"] = csrf_token
        if auth_token: headers["Authorization"] = f"Bearer {auth_token}"
        return session, headers

    def try_replay_captured(page) -> bool:
        if not captured_renew_reqs: return False
        log.info(f"🎯 尝试重放 {len(captured_renew_reqs)} 个捕获到的续期请求...")
        session, base_headers = build_api_session(page)
        for i, req in enumerate(captured_renew_reqs):
            ...
            try:
                ...
                if req.get("headers"):
                    for k, v in req["headers"].items():
                        if k.lower() not in ("host", "content-length", "content-encoding"):
                            hdrs[k] = v
                method = req["method"]; url = req["url"]; post_data = req.get("post_data")
                if method == "GET":
                    resp = session.get(url, headers=hdrs, timeout=15)
                elif method in ("POST", "PUT", "PATCH"):
                    if post_data:
                        resp = session.request(method, url, data=post_data, headers=hdrs, timeout=15)
                    else:
                        resp = session.request(method, url, json={"id": server_id, "serverId": server_id}, headers=hdrs, timeout=15)
                else:
                    resp = session.get(url, headers=hdrs, timeout=15)
                ...
                if resp.status_code in (200, 201, 204):
                    ...
                    if check_expiry_increased():
                        log.info(f"✅ [捕获重放-{i+1}] 续期成功"); return True
            except Exception as e:
                log.warning(f"    重放异常: {e}")
        return False

    def click_modal_confirm(page) -> bool:
        time.sleep(1)
        result = page.evaluate("""() => {
            ...
            var confirmTexts = ['Confirm', 'Yes', 'OK', 'Ok', 'Submit', 'Renew', 'Renew Server', 'Extend', 'Continue', 'Proceed'];
                ...
                for (var b of btns) {
                    ...
                    if (txt === t || txt.toLowerCase() === t.toLowerCase()) {
                        if (b.offsetParent !== null && !b.disabled) {
                            b.scrollIntoView({block: 'center'}); b.focus(); b.click();
                            return 'exact:' + t;
                        }
                    }
                }
            ...
            for (var b of btns) {
                ...
                if ((txt.includes('confirm') || txt.includes('renew') || txt.includes('extend') || txt.includes('proceed'))
                    && b.offsetParent !== null && !b.disabled) {
                    b.scrollIntoView({block: 'center'}); b.focus(); b.click();
                    return 'fuzzy:' + txt;
                }
            }
            var primaryBtn = searchIn.querySelector('button.btn-primary, button.btn-success, button[class*="primary"], button[class*="success"]');
            if (primaryBtn && primaryBtn.offsetParent !== null && !primaryBtn.disabled) {
                primaryBtn.scrollIntoView({block: 'center'}); primaryBtn.click();
                return 'primary_btn';
            }
            return null;
        }""")
        if result:
            log.info(f"  ✅ 已点击模态框确认按钮: [{result}]")
    ...
    for attempt in range(1, 5):
        log.info(f"🔄 [方法1] 尝试 {attempt}/4 - 点击 Renew Server 按钮")
        try:
            page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        except: continue
        time.sleep(3); dismiss_all_popups(page); time.sleep(1)
        take_screenshot(page, f"renew_before_click_attempt{attempt}")
        if attempt == 1:
            sniffed_apis = sniff_renew_api(page)
        btn_clicked = False
        try:
            btn_clicked = page.evaluate("""() => {
                    ...
                    for (var el of els) {
                        ...
                        if (txt === kw || txt.toLowerCase() === kw.toLowerCase()) {
                            if (el.offsetParent !== null && !el.disabled) {
                                el.scrollIntoView({block: 'center'}); el.focus(); el.click(); return kw;
                            }
                        }
                    }
                }
                for (var el of els) {
                    ...
                    if ((txt.includes('renew') || txt.includes('extend')) && el.offsetParent !== null && !el.disabled) {
                        el.scrollIntoView({block: 'center'}); el.focus(); el.click(); return 'fuzzy:' + txt;
                    }
                }
                return null;
            }""")
            if btn_clicked:
                log.info(f"✅ 已通过 JS 点击按钮: {btn_clicked}")
            else:
                log.warning("JS 未找到按钮，尝试 Playwright locator")
                renew_btn = page.locator('a:has-text("Renew Server"), button:has-text("Renew Server"), a:has-text("Renew"), button:has-text("Renew"), a:has-text("Extend"), button:has-text("Extend")').first
                if renew_btn.count():
                    renew_btn.scroll_into_view_if_needed()
                    renew_btn.click(force=True)
                    log.info("✅ 已通过 locator 点击续期按钮"); btn_clicked = True
                else:
                    log.warning(f"第 {attempt} 次：未找到任何续期按钮")
                    take_screenshot(page, f"renew_no_btn_attempt{attempt}"); continue
        except Exception as e:
            log.warning(f"点击失败: {e}"); continue
        ...
        if modal_visible:
            log.info("检测到模态框，尝试点击确认按钮...")
            take_screenshot(page, f"renew_modal_attempt{attempt}")
            confirmed = click_modal_confirm(page)
            if not confirmed:
                try:
                    page.keyboard.press("Tab"); time.sleep(0.5)
                    page.keyboard.press("Enter"); log.info("  回退：Tab + Enter 确认"); time.sleep(2)
                except: pass
                confirmed = click_modal_confirm(page)
        else:
            log.info("未检测到模态框，可能直接触发或按钮未响应")

        log.info("等待 25 秒监听网络请求...")
        time.sleep(25)

        if captured_renew_reqs:
            if try_replay_captured(page):
                page.remove_listener("request", log_req)
                page.remove_listener("response", log_resp)
                return True

        if check_expiry_increased():
            log.info(f"✅ [方法1] 第 {attempt} 次续期成功")
            page.remove_listener("request", log_req)
            page.remove_listener("response", log_resp)
            return True
        else:
            log.warning(f"第 {attempt} 次后 expiry 未变化")
            take_screenshot(page, f"renew_check_attempt{attempt}")

    # ===== 方法2：直接 API 猜测 =====
    log.info("🔄 [方法2] 尝试直接调用可能的续期 API...")
    try:
        page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        time.sleep(3)
        session, base_headers = build_api_session(page)
        sniffed = sniff_renew_api(page)
        get_paths = list(sniffed) + [
            f"{BASE_URL}/server/renew?id={server_id}",
            f"{BASE_URL}/api/server/renew?id={server_id}",
            f"{BASE_URL}/api/v1/server/renew?id={server_id}",
            f"{BASE_URL}/api/v2/server/renew?id={server_id}",
            f"{BASE_URL}/server/extend?id={server_id}",
            f"{BASE_URL}/api/server/extend?id={server_id}",
            f"{BASE_URL}/server/refresh?id={server_id}",
            f"{BASE_URL}/api/servers/{server_id}/renew",
            f"{BASE_URL}/api/servers/{server_id}/extend",
            f"{BASE_URL}/api/servers/{server_id}/refresh",
        ]
        for path in get_paths:
            log.info(f"  GET {path}")
            try:
                resp = session.get(path, headers=base_headers, timeout=12)
                log.info(f"    status={resp.status_code}, body={resp.text[:200]}")
                if resp.status_code in (200, 201, 204):
                    time.sleep(6)
                    if check_expiry_increased():
                        log.info("✅ [方法2-GET] API 续期成功")
                        page.remove_listener("request", log_req)
                        page.remove_listener("response", log_resp)
                        return True
            except Exception as e:
                log.warning(f"  异常: {e}")
        post_paths = [
            f"{BASE_URL}/api/server/renew", f"{BASE_URL}/api/v1/server/renew",
            f"{BASE_URL}/api/v2/server/renew", f"{BASE_URL}/api/server/extend",
            f"{BASE_URL}/api/servers/renew", f"{BASE_URL}/server/renew",
            f"{BASE_URL}/api/servers/{server_id}/renew",
        ]
        body_variants = [
            {"serverId": server_id}, {"id": server_id}, {"server_id": server_id},
            {"serverId": server_id, "action": "renew"}, {"id": server_id, "action": "renew"},
        ...
        for path in post_paths:
            for body in body_variants:
                ...
                try:
                    resp = session.post(path, json=body, headers=base_headers, timeout=12)
                    log.info(f"    status={resp.status_code}, body={resp.text[:200]}")
                    if resp.status_code in (200, 201, 204):
                        time.sleep(6)
                        if check_expiry_increased():
                            log.info("✅ [方法2-POST] API 续期成功")
                            page.remove_listener("request", log_req)
                            page.remove_listener("response", log_resp)
                            return True
                except Exception as e:
        ...
        log.warning(f"[方法2] API 尝试失败: {e}")

    # ===== 方法3：浏览器控制台调用 =====
    log.info("🔄 [方法3] 尝试在控制台执行可能存在的续期函数...")
    try:
        page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        time.sleep(3); dismiss_all_popups(page)
        result = page.evaluate("""() => {
            ...
            var app = document.querySelector('#app') || document.querySelector('[data-server-id]') || document.querySelector('[id^="app"]');
            if (app) {
                var vm = app.__vue__ || app._vei || (app.__vueParentComponent && app.__vueParentComponent.proxy);
                if (vm) {
            ...
            for (var b of btns) {
                ...
                if (txt.includes('renew') || txt.includes('extend')) {
                    if (b.onclick) { b.onclick(new Event('click')); return 'btn.onclick:' + txt; }
                    if (b.click) { b.click(); return 'btn.click:' + txt; }
                }
            }
            return null;
        }""")
        if result:
            log.info(f"  调用了 {result}，等待 15 秒...")
            time.sleep(15)
            handle_cf_turnstile(page, timeout=30)
            click_modal_confirm(page)
            time.sleep(5)
            if check_expiry_increased():
                log.info("✅ [方法3] 控制台调用续期成功")
                page.remove_listener("request", log_req)
                page.remove_listener("response", log_resp)
                return True
    except Exception as e:
        log.warning(f"[方法3] 控制台调用失败: {e}")
    ...
    log.info("=====================================")
    if captured_renew_reqs:
        log.info(f"🎯 已捕获 {len(captured_renew_reqs)} 个疑似续期 API（但重放均未成功）:")
        for r in captured_renew_reqs:
            log.info(f"  {r['method']} {r['url']}")
    else:
        log.info("提示：续期请求未被捕获，请检查是否按钮点击成功，或续期 API 路径需调整。")
    page.remove_listener("request", log_req)
    page.remove_listener("response", log_resp)
    return False
