...
def _try_interact_cf_iframe(page):
    """
    尝试与 CF Turnstile iframe 交互：
    1. 先用 JS 给 iframe 注入点击事件
    2. 再用 Playwright frame 定位 checkbox 并点击
    """
    try:
        page.evaluate("""() => {
            ...
            frames.forEach(function(f) {
                try {
                    f.contentDocument && f.contentDocument.querySelector('input[type="checkbox"]') &&
                    f.contentDocument.querySelector('input[type="checkbox"]').click();
                } catch(e) {}
            ...
            if "challenges.cloudflare.com" in (frame.url or ""):
                try:
                    ...
                    if cb.is_visible(timeout=2000):
                        cb.click(force=True)
                        log.info("  ✅ 已点击 CF Turnstile checkbox（frame）")
                        return
                except Exception:
                    pass
                try:
                    lb = frame.locator('label').first
                    if lb.is_visible(timeout=2000):
                        lb.click(force=True)
                        log.info("  ✅ 已点击 CF Turnstile label（frame）")
                        return
                except Exception:
                    pass
    except Exception as e:
        log.info(f"  CF frame 定位交互: {e}")
...
def wait_cf_turnstile(page, timeout=120) -> bool:
    ...
    多重检测策略 + 主动交互 + 逐步降级：
      A. cf-turnstile-response 有值
      B. 确认按钮 !disabled
      C. 无任何可见 CF iframe（验证已消失/跳过）
      D. 每 20s 主动与 CF iframe 交互一次
      E. 超时兜底：强制 removeAttribute disabled 并提交
    """
    ...
    if not renew_modal_visible:
        log.warning("⚠️ renewModal 未弹出，尝试强力清除广告后重试...")
        force_clear_ads(page)
        time.sleep(2)

    # 2. 首次主动与 CF iframe 交互
    _try_interact_cf_iframe(page)
    time.sleep(2)

    # 3. 主循环等待（最多 timeout 秒）
    ...
    while time.time() < deadline:
        # 每 20s 主动触发一次
        if time.time() - last_interact >= 20:
            log.info("  🔄 主动与 CF iframe 交互（辅助触发验证）...")
            _try_interact_cf_iframe(page)
            last_interact = time.time()
        ...
        passed = page.evaluate("""() => {
            ...
            // 条件C: CF iframe 已不可见（验证完成消失）
            var cfFrames = Array.from(document.querySelectorAll('iframe[src*="challenges.cloudflare.com"]'));
            var visibleCfFrames = cfFrames.filter(function(f) {
                return f.offsetParent !== null || f.getBoundingClientRect().height > 0;
            });
            if (cfFrames.length > 0 && visibleCfFrames.length === 0) return 'iframe_gone';

            // 条件D: Turnstile widget 显示成功状态
        ...
        if passed:
            log.info(f"  ✅ CF Turnstile 验证通过（条件: {passed}）")
            cf_passed = True
            break
        time.sleep(1)

    if not cf_passed:
        log.warning(f"CF Turnstile 验证超时（{timeout}s 内未完成）")

        # 兜底：强制 remove disabled 并提交
        log.info("  🚨 兜底：尝试强制点击确认按钮（忽略 disabled 状态）...")
        force_submitted = page.evaluate("""() => {
            var m = document.getElementById('renewModal') || document.body;
            var keywords = ['renew', 'confirm', 'submit', 'ok', '确认', '续期'];
            var excludes  = ['cancel', 'close', '取消', '关闭'];
            var btns = Array.from(m.querySelectorAll('button, input[type="submit"], a[role="button"]'));
            for (var b of btns) {
                var txt = (b.innerText || b.value || b.textContent || '').trim().toLowerCase();
                if (excludes.some(function(k){ return txt.includes(k); })) continue;
            ...
            var forms = m.querySelectorAll('form');
            if (forms.length > 0) { forms[0].submit(); return 'form_submit'; }
            return null;
        }""")

        if force_submitted:
            log.info(f"  🚨 兜底强制提交: [{force_submitted}]，等待 5s 观察结果...")
            time.sleep(5)
            page_text = get_text(page)
            if any(kw in page_text for kw in ["success", "renewed", "续期成功", "已续期"]):
                log.info("  ✅ 强制提交后页面显示续期成功！")
                return True
            log.warning("  强制提交后未能确认成功，续期可能失败")
        return False
...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    ...
    for cf_attempt in range(1, MAX_CF_ATTEMPTS + 1):
        if cf_attempt == 1:
            ...
            log.warning(f"CF 验证失败，8s 后重新点击 Renew Server（第 {cf_attempt}/{MAX_CF_ATTEMPTS} 次）...")
            time.sleep(8)
            try:
            ...
            if not _click_renew_btn():
                log.warning(f"第 {cf_attempt} 次点击 Renew Server 也失败，继续下一次")
