...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    """
    采用 wait_for_response 主动等待续期 API，并尝试多种触发方式。
    """
    server_url = f"{BASE_URL}/server?id={server_id}"
    # 用于存储捕获的请求 URL（调试）
    captured_requests = []
    ...
    def log_request(request):
        url = request.url
        if "api" in url or "server" in url or "renew" in url or "extend" in url:
    ...
    for attempt in range(1, 3):  # 最多 2 次
        ...
        # 3. 等待模态框出现，但不点击任何按钮，而是尝试点击模态框背景或直接调用 JS
        modal_seen = False
        for _ in range(10):
                ...
                try:
                    ...
                    page.evaluate("""() => {
                        ...
                        if (modal) {
                            ...
                            modal.click();
                            // 也尝试点击其父级的 backdrop
                            var backdrop = modal.closest('.modal-backdrop, .modal-overlay, .modal-wrapper');
                            if (backdrop) backdrop.click();
                        }
                    }""")
                    log.info("  已尝试点击模态框背景以触发确认")
                    time.sleep(2)
                except Exception as e:
                    log.info(f"点击背景失败: {e}")

                # ★ 尝试直接调用页面中可能存在的续期函数（如果暴露在 window 上）
                try:
                    func_called = page.evaluate("""() => {
                        if (window.renewServer && typeof window.renewServer === 'function') {
                            window.renewServer();
                            return true;
                        }
                        // 或者查找按钮的 onclick 处理函数
                    ...
                    if func_called:
                        log.info("✅ 通过 JS 直接调用了续期函数")
                except Exception as e:
                    log.info(f"JS 调用函数失败: {e}")
        ...
        # 4. 处理 CF（如果有）
        handle_cf_turnstile(page, timeout=40)

        # 5. 使用 wait_for_response 主动等待续期 API（超时 30 秒）
        try:
            # 尝试匹配常见的续期 API 模式
            patterns = [
                "**/api/**/renew**",
            ...
            for pattern in patterns:
                try:
                    with page.expect_response(lambda r: pattern in r.url, timeout=30000) as resp_info:
                        # 等待 30 秒内任何匹配的响应
                        pass
                    response = resp_info.value
                    if response and response.ok:
                        log.info(f"✅ 捕获到续期响应: {response.status} {response.url}")
                        break
                except:
                    continue
            if response and response.ok:
                log.info("✅ 续期 API 响应成功，等待刷新...")
                time.sleep(5)
                # 刷新页面验证 expiry
                page.reload()
                time.sleep(3)
                ...
                if parse_expiry_minutes(info_after) > parse_expiry_minutes(expiry_before):
                    log.info("✅ 续期成功（expiry 增加）")
                    page.remove_listener("request", log_request)
                    return True
                else:
                    log.warning("API 响应成功但 expiry 未增加，可能续期条件不满足")
            else:
                log.warning("⏳ 未在 30 秒内捕获到续期 API 响应")
        except Exception as e:
            log.warning(f"等待响应异常: {e}")

        # 6. 如果没等到响应，但可能续期已通过其他方式完成，检查 expiry
        try:
            page.reload()
            time.sleep(3)
            info_after = page.evaluate("""() => {
            ...
            if parse_expiry_minutes(info_after) > parse_expiry_minutes(expiry_before):
                log.info("✅ 续期成功（expiry 增加，尽管未捕获 API）")
                page.remove_listener("request", log_request)
                return True
        except:
            pass
