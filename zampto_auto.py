...
def handle_cf_turnstile(page, timeout=45):
    """处理 Cloudflare Turnstile，增加轮询等待"""
...
def login(page, max_retries=3):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            time.sleep(2)
            # 填写用户名
            user_input = page.locator('input[name="identifier"], input[type="email"]').first
            if user_input.count() == 0:
                log.warning("未找到用户名输入框")
                continue
            user_input.click()
            user_input.fill("")
            user_input.type(USERNAME, delay=random.randint(60, 120))
            human_delay()
            ...
            pass_input.click()
            pass_input.fill("")
            pass_input.type(PASSWORD, delay=random.randint(60, 120))
            human_delay()
            page.locator('button[type="submit"]').first.click()
            # 等待跳转
            if page.wait_for_url("**/dash.zampto.net/**", timeout=20000):
                log.info("✅ 登录成功")
                take_screenshot(page, "login_success")
                return True
            log.warning("登录后未跳转")
        except Exception as e:
            log.warning(f"登录尝试 {attempt} 失败: {e}")
            time.sleep(2)
    log.error("❌ 登录失败")
    return False
...
def start_server(page):
    ...
    for attempt in range(1, 4):
        ...
        for _ in range(18):  # 每 10 秒检查，共 3 分钟
            time.sleep(10)
