...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    ...
    log.info("CF 验证通过，等待后台处理并强制刷新验证到期时间...")
    
    minutes_before = parse_expiry_minutes(expiry_before)
    info_after = expiry_before
    minutes_after = minutes_before
    renew_success = False

    # 【核心优化 1】增加重试次数，延长等待总时长（最多50秒），给 Zampto 后台足够时间更新数据
    for attempt in range(1, 6):
        time.sleep(10)
        take_screenshot(page, f"07_after_renew_attempt_{attempt}")
        
        try:
            # 【核心优化 2】强制破除缓存。在 URL 尾部添加时间戳参数，防止浏览器或 CF 强缓存导致页面不变
            timestamp = int(time.time())
            check_url = f"{BASE_URL}/server?id={server_id}&_t={timestamp}"
            page.goto(check_url, timeout=30000, wait_until="domcontentloaded")
            time.sleep(4) 
            dismiss_all_popups(page)
            time.sleep(1)
        except Exception as e:
            log.warning(f"获取最新状态异常 (第 {attempt} 次): {e}")
            continue
        ...
        log.info(f"第 {attempt} 次检测续期后 expiry: {info_after}")
    ...
    log.info(f"最终结果 - 续期前 expiry 分钟数: {minutes_before}, 续期后: {minutes_after}")
    ...
    if renew_success:
        ...
        log.warning("如果多次执行仍未增加，极大概率是面板限制了【最长续期时间上限】(如达到 1 day 就不让继续叠加)。建议到期前数小时再测。")
        return False
