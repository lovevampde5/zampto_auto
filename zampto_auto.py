...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    """
    使用多种极端手段尝试触发续期：
    1. 点击 Renew Server，然后点击 Cancel，再点击 Renew，重复最多 3 次（模拟重试）
    2. 处理 reCAPTCHA 和 Cloudflare
    3. 监听响应，捕获续期 API
    4. 直接通过 requests 库尝试调用续期 API（猜测路径）
    """
    server_url = f"{BASE_URL}/server?id={server_id}"
    # 存储捕获的响应
    captured_responses = []
    ...
    def handle_response(response):
        nonlocal renew_api_hit
        url = response.url
        if any(k in url.lower() for k in ['renew', 'extend', 'server/update', '/api/server']):
            log.info(f"📡 响应: {response.status} {url}")
            captured_responses.append((response.status, url))
            if response.ok:
                renew_api_hit = True
                log.info(f"✅ 续期 API 响应成功！")

    page.on("response", handle_response)

    # 尝试重复点击 Cancel + Renew 循环
    for cycle in range(1, 4):
        log.info(f"🔄 续期循环 {cycle}/3")
        # 访问详情页
        try:
            page.goto(server_url, timeout=30000, wait_until="domcontentloaded")
        except Exception as e:
        ...
        try:
            ...
            take_screenshot(page, f"renew_clicked_{cycle}")
        except Exception as e:
        ...
        if modal_visible:
            ...
            # 点击 Cancel 按钮（如果有）
            try:
                cancel_btn = page.locator('#renewCancelButton, button:has-text("Cancel")').first
                if cancel_btn.count() and cancel_btn.is_visible():
                    cancel_btn.click()
                    log.info("点击了 Cancel 按钮")
                    time.sleep(2)
                else:
                    # 按 ESC
                    page.keyboard.press("Escape")
                    log.info("按 ESC 关闭模态框")
                    time.sleep(2)
            except Exception as e:
                log.warning(f"关闭模态框失败: {e}")
        else:
            log.warning("未检测到模态框，可能续期已直接触发")

        # 等待一段时间，监听响应
        log.info("⏳ 等待 15 秒，监听续期 API...")
        time.sleep(15)
        ...
        # 如果捕获到续期 API 响应且返回成功，但 expiry 未增，可能是服务器延迟，再等待 10 秒重试
        if renew_api_hit:
            log.info("检测到续期 API 响应成功，但 expiry 未刷新，等待 10 秒再检查...")
            time.sleep(10)
            page.reload()
            ...
            if parse_expiry_minutes(info_after2) > minutes_before:
                log.info("✅ 延时后 expiry 增加，确认成功")
                page.remove_listener("response", handle_response)
                return True

        # 如果未成功，继续下一个循环
        log.warning(f"循环 {cycle} 未成功")

    # 所有循环失败，尝试直接 API 调用（猜测）
    log.info("🔄 尝试直接调用续期 API（猜测）...")
    # 尝试常见的续期 API 路径
    possible_paths = [
                ...
                if resp.status_code == 200:
                    ...
                    if "success" in resp.text.lower() or "ok" in resp.text.lower():
                        ...
                        if parse_expiry_minutes(info_final) > parse_expiry_minutes(expiry_before):
                            log.info("✅ 通过 API 续期成功！")
                            page.remove_listener("response", handle_response)
                            return True
                        else:
                            log.warning("API 返回成功但 expiry 未变化，可能续期无效")
                else:
    ...
    # 所有方法均失败
    page.remove_listener("response", handle_response)
    log.error("❌ 所有续期尝试均失败，请手动检查续期功能是否正常")
    return False
