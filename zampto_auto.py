...
# ---------- ★★★ 深度改进：以 API 请求为主的续期函数 ----------
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    """
    首先尝试从页面中提取续期 API 端点，然后使用 requests 库直接发送请求。
    如果 API 调用失败，再回退到浏览器交互（但之前已失败多次）。
    """
    # 1. 尝试从页面 JavaScript 中查找续期函数或 API 路径
    log.info("🔍 扫描页面 JavaScript 上下文寻找续期线索...")
    js_info = page.evaluate("""() => {
        var result = {};
        // 检查 window 上的常见对象
        var keys = Object.keys(window);
        for (var k of keys) {
            if (k.toLowerCase().includes('renew') || k.toLowerCase().includes('extend') || k.toLowerCase().includes('server')) {
                result[k] = typeof window[k];
            }
        }
        // 检查全局变量
        if (window.server) result['server'] = typeof window.server;
        if (window.api) result['api'] = typeof window.api;
        // 查找包含 'renew' 的全局函数
        var funcs = [];
        for (var k of keys) {
            if (typeof window[k] === 'function' && k.toLowerCase().includes('renew')) {
                funcs.push(k);
            }
        }
    ...
    log.info(f"页面 JS 信息: {js_info}")

    # 2. 获取当前 cookies（用于 API 请求）
    try:
        ...
        log.info("✅ 成功获取 cookies，准备 API 调用")
    except Exception as e:
        log.warning(f"获取 cookies 失败: {e}")
        session = None
    ...
    possible_endpoints = [
        ...
        f"{BASE_URL}/api/v1/server/extend",
        f"{BASE_URL}/api/server/update",
        f"{BASE_URL}/api/v1/server/update",
        f"{BASE_URL}/api/server/{server_id}/renew",
        f"{BASE_URL}/api/v1/server/{server_id}/renew",
    ]
    # 可能还需要包含参数，如 ?id=xxx
    params = {"id": server_id, "serverId": server_id}

    # 4. 尝试 GET 和 POST 请求
    if session:
        for endpoint in possible_endpoints:
            log.info(f"🔄 尝试 API: {endpoint}")
            for method in ['GET', 'POST']:
                try:
                    if method == 'GET':
                        resp = session.get(endpoint, params=params, headers=headers, timeout=10)
                    else:
                        resp = session.post(endpoint, data=params, headers=headers, timeout=10)
                    log.info(f"  {method} {resp.status_code}")
                    if resp.status_code == 200:
                        # 检查响应内容
                        try:
                            data = resp.json()
                            log.info(f"  响应 JSON: {data}")
                            # 如果返回成功，则刷新页面检查 expiry
                            if data.get('success') or data.get('ok') or 'success' in str(data).lower():
                                log.info("✅ API 返回成功，刷新页面检查 expiry...")
                                time.sleep(3)
                                page.goto(f"{BASE_URL}/server?id={server_id}")
                                time.sleep(3)
                                ...
                                if parse_expiry_minutes(info_after) > parse_expiry_minutes(expiry_before):
                                    ...
                                    log.warning("API 返回成功但 expiry 未增加，可能续期未生效")
                        except:
                            # 如果非 JSON，检查文本
                            if "success" in resp.text.lower() or "ok" in resp.text.lower():
                                log.info("✅ API 返回文本包含成功标志，刷新检查...")
                                time.sleep(3)
                                page.goto(f"{BASE_URL}/server?id={server_id}")
                                time.sleep(3)
                                info_after = page.evaluate("""() => {
                        ...
                        log.warning("  认证失败，可能需要重新登录")
                    elif resp.status_code == 404:
                        log.warning("  端点不存在")
                except Exception as e:
                    log.warning(f"  请求异常: {e}")

    # 5. 如果 API 调用均失败，则回退到浏览器交互（尽管已多次失败）
    log.info("🔄 API 调用未成功，回退到浏览器交互（最后尝试）...")
    # 这里可以调用原有的点击循环，但我们已经试过，简单重试一次
    for attempt in range(1, 3):
        log.info(f"浏览器交互尝试 {attempt}/2")
        page.goto(f"{BASE_URL}/server?id={server_id}")
        time.sleep(3)
        dismiss_all_popups(page)
        # 点击 Renew Server
        try:
            ...
            if not btn_clicked:
                log.warning("未找到 Renew Server 按钮")
                continue
        ...
        handle_cf_turnstile(page, timeout=20)
        # 等待并检查 expiry
        time.sleep(10)
        page.reload()
        time.sleep(3)
        ...
        if parse_expiry_minutes(info_after) > parse_expiry_minutes(expiry_before):
            log.info("✅ 浏览器交互续期成功！")
            return True
        log.warning(f"浏览器交互尝试 {attempt} 失败")

    # 所有方法均失败
    log.error("❌ 所有续期方法均失败，请手动检查")
    return False
