...
# Zampto 免费服务器每次续期约延长 24h，冷却窗口大约 23h，设 23*60=1380 分钟
...
def force_clear_ads(page):
    ...
    直接用 JS 隐藏所有高 z-index 的覆盖层，确保续期弹窗能正常弹出并被检测到。
    ...
    try:
        removed = page.evaluate("""() => {
            ...
            // 只隐藏明确的广告 iframe（白名单排除 CF 和 renewModal 内的 iframe）
            document.querySelectorAll('iframe').forEach(function(f) {
                // 保留 Cloudflare Turnstile（src 含 challenges.cloudflare.com）
                if (f.src && f.src.includes('challenges.cloudflare.com')) return;
                // 保留 renewModal 内部的所有 iframe（Turnstile 初始化时 src 可能为空）
            ...
            // 隐藏 Google Adsense（不在 renewModal 内）
...
def wait_cf_turnstile(page, timeout=90) -> bool:
    ...
    # 等待 renewModal 出现（最多 10 秒，可能被广告延迟渲染）
    modal_wait = 10
    ...
    if not renew_modal_visible:
        # 进一步诊断：renewModal 存在于 DOM 中但不可见，vs 根本不存在
    ...
    for i in range(wait_appear):
        has_cf = page.evaluate("""() => {
            // 检测方式1：src 含 challenges.cloudflare.com
            var frames = document.querySelectorAll('iframe');
            for (var f of frames) {
                if (f.src && f.src.includes('challenges.cloudflare.com')) return true;
            }
            // 检测方式2：Turnstile widget 容器（src 初始化前可能为空）
            // Turnstile 通常在 renewModal 内渲染一个 iframe，class 含 cf-turnstile 或 data 属性
            var renewModal = document.getElementById('renewModal');
            if (renewModal) {
    ...
    if not cf_appeared:
        # CF iframe 没出现，但 renewModal 可见：检查是否有可直接点击的提交按钮
    ...
    while time.time() < deadline:
        still_verifying = page.evaluate("""() => {
            // 方式1：检测 CF src 的 iframe 是否还在
            var frames = document.querySelectorAll('iframe');
            for (var f of frames) {
                if (f.src && f.src.includes('challenges.cloudflare.com')) return true;
            }
            // 方式2：检测 renewModal 内是否还有 iframe（Turnstile 完成后会移除）
            var renewModal = document.getElementById('renewModal');
            if (renewModal) {
                var modalFrames = renewModal.querySelectorAll('iframe');
                if (modalFrames.length > 0) {
                    // 如果 modal 内 iframe 的 src 已经不含 CF，也视为验证完成
...
def renew_server(page, server_id: str, expiry_before: str) -> bool:
    """
    核心修复：
    1. 续期前检测冷却期（expiry 剩余 > RENEW_COOLDOWN_MINUTES 则跳过，视为"已续期"）
    ...
    if minutes_before > RENEW_COOLDOWN_MINUTES:
        ...
        return True  # 视为续期成功，推送时会显示正确的 expiry
    ...
    # - 允许 10 分钟时间流逝误差
    # - 续期后至少要有 60 分钟
    TIME_DRIFT_TOLERANCE = 10
    RENEW_MIN_THRESHOLD  = 60
