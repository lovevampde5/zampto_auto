...
def wait_cf_turnstile(page, timeout=90) -> bool:
    ...
    核心修复：不再依赖 iframe 消失，而是直接等待 cf-turnstile-response 的值生成，或者确认按钮被解锁。
    """
    ...
    while time.time() < deadline:
        passed = page.evaluate("""() => {
            // 条件A: CF token 是否已生成
            var cfRes = document.querySelector('[name="cf-turnstile-response"]');
            ...
            if (m) {
                ...
                for (var b of btns) {
                    ...
                    if ((txt.includes('renew') || txt.includes('confirm') || txt.includes('submit')) && !b.disabled) {
                        return true;
                    }
                }
            }
    ...
    if not cf_passed:
        log.warning(f"CF Turnstile 验证超时（或确认按钮未在 {timeout}s 内解锁）")
        return False
