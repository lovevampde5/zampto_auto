...
def read_expiry_from_page(page) -> str:
    """多种方式读取页面上的 expiry，返回原始字符串或 None"""
    try:
        result = page.evaluate("""() => {
            var body = document.body.innerText || '';
            // 方式1: 标准 Expiry: xxx 格式
            var m = body.match(/Expiry[^:\\n]*:\\s*([^\\n]+)/i);
            if (m && m[1].trim()) return m[1].trim();
            // 方式2: Next Renewal: xxx
            m = body.match(/Next Renewal[^:\\n]*:\\s*([^\\n]+)/i);
            if (m && m[1].trim()) return m[1].trim();
            // 方式3: 直接找时间格式 如 "22h 14m" / "1 day 22h"
...
def wait_cf_turnstile(page, timeout=120) -> bool:
    """
    等待 CF Turnstile 验证完成。
    返回 True 表示验证通过（确认按钮已点击）；False 表示超时放弃。
    【本版本移除了兜底 form_submit，防止误触发导致假成功】
    """
    ...
    if not renew_modal_visible:
        log.warning("⚠️ renewModal 未弹出，尝试强力清除广告后继续等待...")
...
def read_expiry_after_renew(page, server_id: str, minutes_before: int) -> tuple:
    """
    续期后重新导航并多次尝试读取 expiry。
    返回 (expiry字符串, minutes整数)
    多次重试是因为网站可能需要时间更新数据。
    """
